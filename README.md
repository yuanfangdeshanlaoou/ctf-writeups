...
完整解题链路见各题 md，每篇末尾附"实战映射"，说明该技术点在真实场景（水控 / IoT / 金融）里的对应。

## 核心方法论沉淀

1. 先找现成项目/工具再动手，禁止从零硬干
2. 鉴别实验先于方向猜测：请求被无视时，"原样重放 vs 改内容重发"一次实验分清内容校验 / 身份过滤
3. 参数字节长度特征往往是破解钥匙：30 万+组合盲爆全灭后，转攻"哪个参数是唯一 10 字节证据"一次命中
4. 胜利以实际到账为准：宣布成功前必须交叉终验（平台 isSolved / 资金状态）
5. 动态仿真优先于静态啃汇编：unicorn 直接执行目标函数读内存输出，不逐行反汇编
