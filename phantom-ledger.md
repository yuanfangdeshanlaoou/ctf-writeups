...
一个本地 EVM 链 (chain_id 31337, Foundry/Anvil 风格) + PhantomVault 合约。玩家作为 relayer，要让 Setup 合约 `isSolved()` 返回 true。本质是**权限控制缺陷**：`transferCredit(from, to, amount)` 的调用者被当作 relayer，而 Setup 已授权给 relayer——于是玩家直接以 relayer 身份把 Setup 的 10 ETH 转给自己再 `withdraw`。
...
1. **没有库就是最好的库**——手搓 ECDSA/RLP 全程无第三方依赖，链交互只走 `jsonrpc` HTTP，这在一个"沙箱 curl 受限"的环境里是保命技能。
...
- **无依赖链上交互**：在生产环境没有装好的 web3 库、或需要审计 SDK 行为时，能手搓签名/RLP = 不依赖任何中间件直接操作链。对链上资金安全的终验（到账才算）也是真实交易的标准。
